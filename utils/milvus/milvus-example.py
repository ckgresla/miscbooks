...
def main():
    ...
    # 10000 vectors with specified dimension
    # element per dimension is float32 type
    # vectors should be a 2-D array (matrix!)
    vectors = [[random.random() for _ in range(_DIM)] for _ in range(10)]
    #print(vectors)
    # You can also use numpy to generate random vectors:
    #   vectors = np.random.rand(10000, _DIM).astype(np.float32)
    print(f"Generated {len(vectors)} Vectors, inserting into Collection: {collection_name}")
